...
class AzureBlobProvider {
  ...
  async uploadBlob({ buffer, blobName, contentType }) {
    try {
      ...
      if (!connectionString) {
        throw new Error('Azure configuration is missing AZURE_STORAGE_CONNECTION_STRING.');
      }

      if (!containerName) {
        throw new Error('Azure configuration is missing AZURE_STORAGE_CONTAINER_NAME.');
      }
      ...
      throw new Error(`AzureBlobProvider.uploadBlob failed for "${blobName || 'unknown'}": ${message}`);
    }
  }
