...
const attachScope = (fieldName = "society") => {
  return async (req, res, next) => {
    try {
      ...
      const userScope =
        req.userScope || (await resolvePermissions(req.user._id)).scope;
      const userRole =
        req.userRole || (await resolvePermissions(req.user._id)).role;

      if (
        userRole === "sb_faculty_advisor" ||
        userScope.type === "global" ||
        userScope.type === "student_branch"
      ) {
        req.scopeFilter = {};
