...
export default function EventFormPage() {
  ...
  useEffect(() => {
    ...
    eventService.getEvent(id).then((res) => {
      ...
      reset({ title: e.title, description: e.description || '', date: e.date || '', location: e.location || '', status: e.status });
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-heading-1 font-bold text-ink mb-6">{isEdit ? 'Edit Event' : 'New Event'}</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="bg-surface rounded-lg border border-hairline p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-body-sm font-medium text-ink-secondary mb-1.5">Date</label>
            <input type="date" {...register('date')} className="w-full px-3 py-2 bg-surface border border-hairline rounded-xs text-body-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
          </div>
          <div>
            <label className="block text-body-sm font-medium text-ink-secondary mb-1.5">Location</label>
            <input {...register('location')} className="w-full px-3 py-2 bg-surface border border-hairline rounded-xs text-body-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
          </div>
        </div>
        <div>
