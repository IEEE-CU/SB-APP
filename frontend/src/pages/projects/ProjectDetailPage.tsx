      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <AnimatedCard className="bg-surface/60 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-2xl p-6 shadow-sm space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <AnimatedBadge className="px-3 py-1 rounded-full text-[10px] font-bold tracking-widest bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 border border-indigo-500/20 uppercase">
          ...
          <div className="space-y-1.5 pt-2">
            ...
            <div className="w-full bg-canvas-soft/50 h-2.5 rounded-full overflow-hidden border border-white/20 dark:border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                {milestones.map((m, idx) => (
                  <div
                    ...
                    className="p-3.5 rounded-lg border border-white/20 dark:border-white/5 bg-canvas-soft/50 hover:bg-surface/80 cursor-pointer transition-colors flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      {m.completed ? (
          ...
          <div className="space-y-6">
            ...
            <AnimatedCard className="bg-surface/60 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-xl p-6 space-y-4 shadow-sm">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(project.techStack && project.techStack.length > 0 ? project.techStack : ['React', 'TypeScript', 'Node.js', 'PostgreSQL']).map((tech, i) => (
                    <span key={i} className="px-2.5 py-1 bg-canvas-soft/50 border border-white/20 dark:border-white/5 rounded-md text-body-xs font-semibold text-ink">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-white/20 dark:border-white/5 space-y-2">
                {project.githubUrl ? (
                  <a href={project.githubUrl} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-canvas-soft/50 hover:bg-surface/80 border border-white/20 dark:border-white/5 rounded-md text-body-sm font-semibold text-ink transition-colors">
                    <Github size={16} /> Repository
                  </a>
                ) : (
                  <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-canvas-soft/50 hover:bg-surface/80 border border-white/20 dark:border-white/5 rounded-md text-body-sm font-semibold text-ink transition-colors">
            ...
            {/* Team Members */}
            <AnimatedCard className="bg-surface/60 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-xl p-6 space-y-3">
              ...
              <div className="space-y-2 text-body-sm">
                <div className="p-2 rounded bg-canvas-soft/50 font-medium text-ink">Project Lead & Architect</div>
                <div className="p-2 rounded bg-canvas-soft/50 text-ink-muted">Core Contributors ({project.memberIds?.length || 3})</div>
              </div>
            </AnimatedCard>
          </div>
        </div>
